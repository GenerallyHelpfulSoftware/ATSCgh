...
{
    NSMutableString* mutableResult = [[NSMutableString alloc] initWithCapacity:512];
...
{
    NSString* result = [NSString stringWithFormat:@"%@.%@", self.virtualMajorChannelNumber.stringValue, self.virtualMinorChannelNumber.stringValue];
    return result;
}
...
-(NSString*) userVisibleName
{
    ...
    NSString* result = [self completedSubchannelNumber];
...
-(Network*)network
{
    Network* result = self.subchannel.network;
...
{
    NSString* result = self.programName;
    Network* myNetwork = self.network;
